        ...
        <tbody>
            ...
            foreach ($exportData as $r) {
                if ($current_jenis !== $r['kode_jenis']) {
                    ...
                    <tr>
                        <td></td>
                        <td style="text-align:right; font-weight:bold;"><?=$jenis_no++?></td>
    ...
    <div class="card">
      <div class="card-header p-3">
        <div class="text-center">
          <div class="fw-bold" style="font-size:1rem">BUKU PENERIMAAN BARANG PERSEDIAAN</div>
        ...
        <table class="table table-bordered table-sm" style="font-size:.82rem">
          <thead>
            <tr class="table-primary text-center">
              <th>No</th><th>Jenis Barang</th><th>Kode Barang</th><th>Nama Barang</th>
              <th>Dokumen Faktur</th><th>Banyaknya</th><th>Satuan</th>
              <th>Harga (Rp)</th><th>Jumlah (Rp)</th><th>Bukti Penerimaan</th>
            </tr>
          </thead>
          <tbody>
            <?php
            $no=1; $totalJumlah=0; $totalQty=0; $found=false;
            while($r=$data->fetch_assoc()): $found=true;
            $totalJumlah+=$r['jumlah_harga']; $totalQty+=$r['jumlah'];
            ?>
            <tr>
              ...
              <td class="text-end fw-semibold"><?=formatRupiah($r['jumlah_harga'])?></td>
              <td><?=htmlspecialchars($r['no_bukti_penerimaan']??'—')?></td>
            </tr>
            <?php endwhile; if(!$found): ?>
            <tr><td colspan="10" class="text-center text-muted py-4"><i class="bi bi-inbox me-2"></i>Tidak ada data untuk filter yang dipilih.</td></tr>
            <?php endif; ?>
          </tbody>
          <?php if($found): ?>
          <tfoot>
            <tr class="table-secondary fw-bold">
              ...
              <td></td><td></td>
              <td class="text-end text-success"><?=formatRupiah($totalJumlah)?></td>
              <td></td>
            </tr>
          </tfoot>
          <?php endif; ?>
